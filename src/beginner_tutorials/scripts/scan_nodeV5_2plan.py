...
def scan_node():
    ...
    while not rospy.is_shutdown():
        ...
        if j < len(world_angles_mat[0]):  #print(angles_mat[:, j])
            ...
            create(joints_angles_rad, 'all_angles.csv')
            create(current_angles, 'active_angles.csv')    # write to file
        if j == len(world_angles_mat[0])/2 and flag:
            delayForceCalc = 0
            while delayForceCalc < 30:
                print(j)
                current_angles = [2600, 1800, 3000]
                create(current_angles, 'active_angles.csv')
                angles_msg.data = current_angles
                pub1.publish(angles_msg)
                delayForceCalc += 1
                time.sleep(1)
